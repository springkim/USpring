...
system "mkdir build";
chdir("build");
system "cmake ../opencv-3.3.0 -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=build -DWITH_TBB=ON -DBUILD_TESTS=OFF _DBUILD_PERF_TESTS=OFF -DBUILD_opencv_world=OFF -DOPENCV_EXTRA_MODULES_PATH=../opencv_contrib-3.3.0/modules -DWITH_FFMPEG=ON -DWITH_QT=ON";
